...
const getHealthStati = async (adapter: ioBroker.Adapter): Promise<T_AdapterStates> => {
	const singleAStates = await AdapterUtilsFunctions.getAdapterSingleStates(adapter, 'netatmo');
...
const rootLevelElementsCreator = async (adapter: ioBroker.Adapter): Promise<T_RearangeDeviceAndStates_Props[]> => {
	...
	const rootLevelPath = await AdapterUtilsFunctions.getAdapterPath(adapter, 'netatmo');
	...
	if (rootLevelElements) {
		for (const rootObj of Object.values(rootLevelElements)) {
			const [deviceType, room] = rootObj._id.substr(rootObj._id.lastIndexOf('.') + 1).split('-');
			returnStruct.push({ deviceType, room, rootObj });
		}
	}
	return returnStruct;
};
