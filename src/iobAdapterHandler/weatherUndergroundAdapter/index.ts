...
const name = 'WeatherundergroundAdapter';
const adapterName = 'weatherunderground';
...
const getHealthStati = async (adapter: ioBroker.Adapter): Promise<T_AdapterStates> => {
	...
	const observeTimeState = await adapter.getForeignStateAsync(
		`${instancePath}.forecast.current.observationTimeRFC822`,
	);
	if (observeTimeState && typeof observeTimeState.val === 'string') {
		const observeTime = new Date(observeTimeState.val);
		const now = Date.now();
	...
	throw new Error(
		`Last Update to long ago (more than ${adapter.config.WeatherundergroundAdapter_lastObeserveTimeSinceMinutes} min), is the adapter still running and is it installed ?`,
	);
};
...
const WeatherundergroundAdapter: T_IOBAdapter_Handler = {
	name: name,
	isHealth: isHealth,
	onMessageFunc: { getHealthStati: getHealthStati, rename: rename },
};

export default WeatherundergroundAdapter;
