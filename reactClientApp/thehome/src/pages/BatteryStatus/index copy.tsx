		...
		genericApp.socket.getState('thehome.0.batteryStati').then((state) => {
			if (state && state.val && typeof state.val == 'string') {
				...
				for (const stat of filteredV) {
					if (stat.value <= (genericApp.state.native as any)['BatteryChecker_lowBatPercent_alert']) {
						(_rowPropsV as any)[stat.name] = { background: ['status-critical'], pad: 'small' };
					}
					if (
						stat.value <= (genericApp.state.native as any)['BatteryChecker_lowBatPercent_warning'] &&
						stat.value > (genericApp.state.native as any)['BatteryChecker_lowBatPercent_alert']
					) {
